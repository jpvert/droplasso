...
for (proba in probalist) {
  ...
  m <- droplasso(data_train$x, data_train$y, family="binomial", keep_prob=proba, nlambda=20, n_passes = 10000)
...
cat('.')
  # Train glmnet model
  m_glm <- glmnet(data_train$x, data_train$y, family="binomial", intercept=F, alpha=alpha, standardize = F)
